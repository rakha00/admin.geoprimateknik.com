...
<body>
  ...
  <table style="margin-top:-70px;">
    <tr>
      ...
      <td>No Invoice</td>
      <td>{{ $transaksi->no_invoice_non_pajak }}</td>
    </tr>
    <tr>
      <td style="border:none;"></td>
      <td>Tanggal</td>
      <td style="font-size: 11px">{{ \Carbon\Carbon::parse($transaksi->tanggal)->translatedFormat('d F Y') }}</td>
    </tr>
  </table>
  <br>
   <hr><hr>
  ...
  <table style="margin-top:15px;">
    ...
    @foreach($transaksi->details as $i => $d)
      ...
      <tr>
        ...
        <td>{{ $d->nama_unit }} <br> {{ $d->remarks ?? '' }}</td>
